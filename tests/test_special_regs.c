...
static void test_gpio_forced_output_latch(void) {
    ...
    gpio_write(pattern);
    ASSERT((gpio_read() & GPIO_FORCED_OUTPUT_PINS) == (pattern & GPIO_FORCED_OUTPUT_PINS));
}

static void test_gpio_direction_mask(void) {
    gpio_write_direction(0x00000000u);
    ASSERT((gpio_read_direction() & GPIO_FORCED_OUTPUT_PINS) == GPIO_FORCED_OUTPUT_PINS);
    ASSERT((gpio_read_direction() & GPIO_CONFIGURABLE_PINS) == 0u);
}

static void test_gpio_byte_store(void) {
    gpio_write_direction(GPIO_ALL_PINS);
    gpio_write(0x00000000u);
    gpio_write8(0u, 0x5Au);
    gpio_write8(2u, 0xC3u);
    ASSERT((gpio_read() & 0x00FF00FFu) == 0x00C3005Au);
}

static void test_gpio_input_pin8(void) {
    const uint32_t pin8 = GPIO_PIN(8u);

    gpio_configure_input(pin8);
    ASSERT((gpio_read_direction() & pin8) == 0u);
